...
using System.Linq;
using System.Web;
using ProductsToPinterest;
using Databox.Libs.ProductsToPinterest;
...
using ProductsToPinterest.Helpers;
using ProductsToPinterest.DataItems;
using System.IO;
using System.Text.RegularExpressions;
using ProductsToPinterest.Extensions;
using ProductsToPinterest.Enums;

namespace WheelsScraper
{
    public class ProductsToPinterest : BaseScraper
    {
        ...
        public ProductsToPinterest()
        {
            Name = "ProductsToPinterest";
            Url = "https://www.ProductsToPinterest.com/";
            ...
            Complete += ProductsToPinterest_Complete;
        }

        private void ProductsToPinterest_Complete(object sender, EventArgs e)
        {
        ...
        protected void PinterestProductsProcessed()
        {
            ...
            try
            {
                ...
                List<ExportItem> sceExportItems = new List<ExportItem>();
                foreach (string sceFile in sceFiles)
                {
                    sceExportItems.AddRange(FileHelper.ReadExportFile(sceFile));
                }

                MessagePrinter.PrintMessage($"SCE export Readed... please wait");
                sceExportItems = sceExportItems.Distinct(ExportItem.ProdIdBrandComparer).ToList();
                    ...
                    if (extSett.UseBusinessesAccount)
                    {
                        if (extSett.ChangeWebsite)
                        {
                            ...
                            foreach (var item in PinterestItems)
                            {
                                ...
                                {
                                    var sceItem = sceExportItems.Find(i => i.Brand == item.Brand && i.PartNumber == item.PartNumber);
