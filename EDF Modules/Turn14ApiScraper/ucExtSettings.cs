...
namespace Databox.Libs.Turn14ApiScraper
{
    public partial class ucExtSettings : XtraUserControl
    {
        ...
        private void buttonEditSemadatacoop_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            string filePath = GetFilePath();

            if (!string.IsNullOrEmpty(filePath))
                buttonEditSemadatacoop.Text = filePath;
            else
                throw new Exception("You must select file");
