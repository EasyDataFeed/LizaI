...
using System.Linq;
using Turn14ApiScraper;
using Databox.Libs.Turn14ApiScraper;
using Turn14ApiScraper.DataItems.Turn14;
using Turn14ApiScraper.Helpers;
using System.IO;
using System.Net;
...
{
    public class Turn14ApiScraper : BaseScraper
    {
        ...
        public Turn14ApiScraper()
        {
            Name = "Turn14ApiScraper";
        ...
        protected override void RealStartProcess()
        {
            ...
            Wares.Clear();
            CheckTurn14Setting();
            extSett.SemaTurn14Images.Clear();

            if (extSett.BrandsForScraping.data.Any())
            {
                if (extSett.DoScrapeSemadatacoop)
                {
                    if (string.IsNullOrEmpty(extSett.Semadatacoop))
                    {
                    ...
                    if (tokenSemaJson == null)
                    {
                        if (!string.IsNullOrEmpty(errorST))
                        {
                            MessagePrinter.PrintMessage($"Error while received Sema Token - {errorST}", ImportanceLevel.Critical);
                        }
                        else
                        {
                            MessagePrinter.PrintMessage("Error while received Sema Token", ImportanceLevel.Critical);
                        }

                        return;
                    }

                    extSett.BrandCode = new List<BrandsCodeFile>();
                    extSett.BrandCode = FileHelper.BrandsCodeFiles(extSett.Semadatacoop);

                    foreach (var brand in extSett.BrandsForScraping.data)
                    {
                        foreach (var item in extSett.BrandCode)
                        {
                            if (brand.attributes.name == item.BrandNameTurn14)
                            {
                                MessagePrinter.PrintMessage($"Get info for Sema brand - [{item.AAIABrandId}]");

                                DigitalAssetsJson digitalAssetsJson = SemaApiHelper.GetDigitalAssets(tokenSemaJson.token, item.AAIABrandId, out string errorDA);

                                if (digitalAssetsJson == null)
                                {
                                    if (!string.IsNullOrEmpty(errorDA))
                                    {
                                        MessagePrinter.PrintMessage($"Error while received Sema Digital Assets for brand [{item.AAIABrandId}] - {errorST}", ImportanceLevel.High);
                                    }
                                    else
                                    {
                                        MessagePrinter.PrintMessage($"Error while received Sema Digital Assets for brand [{item.AAIABrandId}]", ImportanceLevel.High);
                                    }

                                    continue;
                                }

                                MessagePrinter.PrintMessage($"Info for Sema brand - [{item.AAIABrandId}] received");

                                extSett.SemaTurn14Images.Add(new SemaTurn14Images
                                {
                                    Name = brand.attributes.name,
                                    Digital = digitalAssetsJson
                                });
                            }
                        }
                    }
        ...
        private void PopulateItemInfo(FullItemInfoJson fullItem, TransferInfoItem transferInfoItem)
        {
            ...
            try
            {
                ...
                if (extSett.DoScrapeSemadatacoop)
                {
                    ...
                    {
                        foreach (var item in extSett.SemaTurn14Images)
                        {
                            if (item.Name == brand)
                            {
                                foreach (var assets in item.Digital.digitalAssets)
                                {
                                    if (manufacturerPartNumber == assets.PartNumber)
                                    {
                                        generalImage = assets.Link;
                                    }
                                }
                            }
                        }
